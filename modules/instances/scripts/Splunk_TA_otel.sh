...
splunk_total_mib_name="splunk_total_mib"
splunk_total_mib_value=""
...
splunk_TA_otel_read_configs() {
    ...
    do
        ...
        has_splunk_total_mib="$(echo "$line" | grep "$splunk_total_mib_name")"
        if [ "$has_splunk_total_mib" ] ; then
            splunk_TA_otel_log_msg "DEBUG" "reading $splunk_total_mib_name from line $has_splunk_total_mib"
            splunk_total_mib_value="$(echo "$has_splunk_total_mib" | grep -Eo ">(.*?)<" | sed 's/^>\(.*\)<$/\1/')"
            splunk_TA_otel_log_msg "INFO" "Set $splunk_total_mib_name to $splunk_total_mib_value"
        fi
        ...
        if [ "$has_splunk_gateway_url" ] ; then
            ...
            splunk_TA_otel_log_msg "INFO" "Set $splunk_gateway_url_name to $splunk_gateway_url_value"
        fi
...
is_splunk_TA_otel_agent_running() {
    ...
    agent_running="$(ps aux | grep -v grep | grep  "$splunk_otel_process_name")"
...
splunk_TA_otel_run_agent() {
    ...
    splunk_gateway_url_value="$(Splunk_TA_otel_expand_config_param "$splunk_gateway_url_value")"
    ...
    if [ "$splunk_config_value" ] ; then
      ...
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_config_name"
    fi
    if [ "$splunk_config_dir_value" ] ; then
      export SPLUNK_CONFIG_DIR="$splunk_config_dir_value"
    else
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_config_dir_name"
    fi
    ...
    if [ "$splunk_debug_config_server_value" ] ; then
      ...
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_debug_config_server_name"
    fi
    if [ "$splunk_config_yaml_value" ] ; then
      export SPLUNK_CONFIG_YAML="$splunk_config_yaml_value"
    else
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_config_yaml_name"
    fi
    ...
    if [ "$splunk_memory_limit_mib_value" ] ; then
      ...
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_memory_limit_mib_name"
    fi
    if [ "$splunk_total_mib_value" ] ; then
      export SPLUNK_TOTAL_MIB="$splunk_total_mib_value"
    else
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_total_mib_name"
    fi
    ...
    if [ "$splunk_realm_value" ] ; then
      ...
      splunk_TA_otel_log_msg "INFO" "NOT SET: $splunk_realm_name"
    fi
...
extract_bundle() {
    if [ -d "$SPLUNK_BUNDLE_DIR" ]; then
        splunk_TA_otel_log_msg "INFO" "Agent bundle path exists on filesystem -- not attempting to extract"
        return 0
    else
